...
static gboolean img_to_boot_emmc_handler(RaucImage *image, RaucSlot *dest_slot, const gchar *hook_name, GError **error) {
	...
	ret = r_emmc_read_bootpart(dest_slot->device, &part_active, &ierror);
	if (!ret) {
		g_propagate_error(error, ierror);
		res = FALSE;
		goto out;
	}
	g_message("Found active eMMC boot partition %sboot%d",
			dest_slot->device, part_active);

	/* create a temporary RaucSlot with the actual partition as device */
	part_slot = g_new0(RaucSlot, 1);

	/* determine active boot partition and set the other to be written */
	...
	g_message("Disabling read-only mode of slot device partition %s",
			part_slot->device);
	...
	/* open */
	g_message("Opening slot device %s", part_slot->device);
	...
	g_message("Reenabling read-only mode of slot device partition %s",
			part_slot->device);
	 ...
	 *
	 * Do this explicitly on determined boot partition to force the kernel
	 * to switch to the partition.
	 */
	g_message("Toggling active eMMC boot partition %sboot%d -> %s",
			dest_slot->device, part_active, part_slot->device);
	 ...
	 */
	ret = r_emmc_read_bootpart(dest_slot->device, &part_active_after, &ierror);
	if (!ret) {
		g_propagate_error(error, ierror);
		res = FALSE;
		goto out;
